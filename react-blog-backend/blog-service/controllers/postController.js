...
exports.deletePost = async (req, res) => {
  try {
    const targetPost = await Post.findById(req.params.id).populate("author");
    if (!targetPost) {
      return res.status(404).json({ message: "Post not found" });
    } else {
      console.log("X", targetPost.author.toString());
      if (req.user.id !== targetPost.author.toString()) {
        return res
          .status(403)
          .json({ message: "You are not authorized to delete this post" });
      }
      console.log("X");
      await Like.deleteMany({ post: targetPost._id });
      ...
      return res.status(200).json({
        message:
          "The post and its associated data have been deleted successfully",
      });
    }
  } catch (error) {
...
exports.updatePost = async (req, res) => {
  ...
  try {
    ...
    console.log("This is the post from the database:");
    console.log(updatedPost);
    const updatedData = {};
    if (!updatedPost) {
      return res.status(404).json({ message: "Post not found" });
    } else {
      console.log(updatedPost.author.toString(), req.user.id);
      if (req.user.id !== updatedPost.author.toString()) {
        console.log(
      ...
      if (newTitle) {
        ...
        console.log("This is the updated title:", updatedData.title);
      }
      if (newContent) {
